...
export class StudentFormComponent implements OnDestroy {
  ...
  classes: string[] = ['LP-DIM-FI', 'LP-DIM-APP'];
  ...
  onSubmit() {
    if (this.studentForm.valid) {
      this.studentForm.value.dateOfBirth = new Date(
        this.studentForm.value.dateOfBirth
      ).toISOString();
      if (this.data.isCreateForm) {
        this.studentForm.value.id = Date.now() + Math.random();
        this.studentService
          .create(this.studentForm.value as Student)
        ...
        this.studentService
          .update(this.studentForm.value as Student)
          .pipe(takeUntil(this.destroy$))
          .subscribe((result) => {
            this._snackBar.open(result, '', {
              duration: 2000,
